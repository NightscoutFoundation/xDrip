            ...
            for (long THIS_QUEUE : circuits) {
                        ...
                        for (UploaderQueue up : bgups) {
                            switch (up.action) {
                                ...
                                case "create":
                                    ...
                                    break;
                                case "delete":
                                    if ((THIS_QUEUE == UploaderQueue.NIGHTSCOUT_RESTAPI) && type.equals(Treatments.class.getSimpleName())) {
                                        items.add(up);
                ...
                if ((bgReadings.size() > 0) || (calibrations.size() > 0) || (bloodtests.size() > 0)
                    ...
                    if (THIS_QUEUE == UploaderQueue.MONGO_DIRECT) {
                        Log.e(TAG, "Unsupported QueueType MONGO_DIRECT - only RESTAPI is implemented!");
                        ...
                        Log.e(TAG, "Unsupported QueueType INFLUXDB_RESTAPI - only RESTAPI is implemented!");
                    }
