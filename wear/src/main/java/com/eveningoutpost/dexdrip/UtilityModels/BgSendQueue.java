...
public class BgSendQueue extends Model {
    ...
    */
    ...
    public static void resendData(Context context) {//KS
        ...
        if (!graph_bgs.isEmpty()) {
            ...
            if (sharedPrefs.getBoolean("show_wear_treatments", false)) {
                DataMap treatsDataMap = getTreatments(startTime);
    ...
    public static DataMap getTreatments(long startTime) {
        ...
        if (last != null) {
            Log.d("BgSendQueue", "getTreatments last.timestamp:" +  JoH.dateTimeText(last.timestamp));
        }
        List<Treatments> graph = Treatments.latestForGraph(60, startTime);
        if (!graph.isEmpty()) {
            Log.d("BgSendQueue", "getTreatments graph size=" + graph.size());
            final ArrayList<DataMap> dataMaps = new ArrayList<>(graph.size());
            DataMap entries = null;
            if (includeTreatment(last)) entries = dataMap(last);
            for (Treatments data : graph) {
                if (includeTreatment(data)) {
                    if (entries == null)
                        entries = dataMap(data);
                    else
            ...
            if (entries != null) {
                entries.putDataMapArrayList("entries", dataMaps);
                Log.d("BgSendQueue", "getTreatments entries=" + entries);
            }
            return entries;
        }
        else return null;
    ...
    public static DataMap getCalibrations(long startTime) {
        ...
        if (last != null) {
            Log.d("BgSendQueue", "getCalibrations last.timestamp:" +  JoH.dateTimeText(last.timestamp));
        }
        ...
        if (!graph.isEmpty()) {
            Log.d("BgSendQueue", "getCalibrations graph size=" + graph.size());
            final ArrayList<DataMap> dataMaps = new ArrayList<>(graph.size());
            DataMap entries = null;
            if (last.slope_confidence != 0) entries = dataMap(last);
            ...
            if (entries != null) {
                entries.putDataMapArrayList("entries", dataMaps);
                Log.d("BgSendQueue", "getCalibrations entries=" + entries);
            }
            return entries;
        }
        else return null;
    }
    private static DataMap dataMap(Calibration data) {
        DataMap dataMap = new DataMap();
        dataMap.putDouble("timestamp", data.timestamp);
    ...
    public static DataMap getBloodTests(long startTime) {
        ...
        if (last != null) {
            Log.d("BgSendQueue", "getBloodTests last.timestamp:" +  JoH.dateTimeText(last.timestamp));
        }
        List<BloodTest> graph = BloodTest.latestForGraph(60, startTime);
        if (!graph.isEmpty()) {
            Log.d("BgSendQueue", "getBloodTests graph size=" + graph.size());
            final ArrayList<DataMap> dataMaps = new ArrayList<>(graph.size());
            ...
            entries.putDataMapArrayList("entries", dataMaps);
            Log.d("BgSendQueue", "getBloodTests entries=" + entries);
            return entries;
        }
        else {
            Log.d("BgSendQueue", "getBloodTests no entries for startTime=" + JoH.dateTimeText(startTime));
            return null;
        }
    }
