...
| `app/src/main/java/com/eveningoutpost/dexdrip/utilitymodels/Notifications.java` | Modified | CHG8, CHG15 | Snooze line ("Alert &lt;name&gt; snoozed until HH:mm") in the ongoing BG notification, with threshold/vehicle-mode/stale-data suppression and prompt refresh on snooze-state changes; CHG15: shared literal helper `snoozeLineText()` |
| `app/src/main/java/com/eveningoutpost/dexdrip/EditAlertActivity.java` | Modified | CHG7/A1, CHG9, CHG15 | REMOVE ALERT stops/clears a matching active alert and rebuilds notifications; test alert fires after a 5-second delay; CHG15: toast text as literal |
...
| `app/src/main/java/com/eveningoutpost/dexdrip/Home.java` | Modified | CHG3, CHG4 (+Addendum A), CHG13/ER1, CHG15 | `onKeyDown()` delegates to the shared volume-key handler; consume decision (no volume change during alarm); new `onKeyUp()` override; ER1: startup prompt when the overlay permission is missing; CHG15: dialog text as literal |
| `app/src/main/java/com/eveningoutpost/dexdrip/utils/Preferences.java` | Modified | CHG1, CHG15 | On enabling *Snooze screen over other apps* without the permission: toast + open the system screen "Display over other apps"; CHG15: toast text as literal |
| `app/src/main/java/com/eveningoutpost/dexdrip/utilitymodels/StatusLine.java` | Modified | CHG5, CHG12, CHG15 | Extra-status-line segment while a snoozed active alert exists; CHG12/CHG15: shows the alert name via the shared `Notifications.snoozeLineText()` helper (identical to the notification line) |
...
| `app/src/main/res/xml/pref_notifications.xml` | Modified | CHG1, CHG11, CHG13/ER1, CHG15 | Switch `snooze_over_other_apps` under *Glucose alerts settings* (default on since ER1); CHG11: moved directly above *Buttons silence alarms*, which now has `android:dependency` on it; CHG15: option texts as English literals (incl. the corrected buttons summary) |
| `app/src/main/res/xml/pref_advanced_settings.xml` | Modified | CHG5, CHG15 | Checkbox `status_line_snoozed_alert` ("Inform on snoozed alert") in the *Extra Status Line* menu; CHG15: option texts as English literals |
| `app/src/main/res/values/strings.xml` | **Reverted to upstream** (CHG15) | CHG15 | All new string resources of CHG1–CHG13 removed and the reworded `volume_buttons_snooze` restored (translations untouched); the texts live on as English literals in the XML/Java files pending the follow-up strings PR — this file is no longer part of the change set |
...
*(CHG15: `strings.xml` is no longer part of the change set — its commit falls away.)*
...
| `Documentation/changes/CHG15-remove-string-resources.md` | **Added** (CHG15) | CHG15 | All new string resources removed per PR #4623 review feedback; texts as English literals pending a follow-up strings PR |
...
- **CHG15** — `strings.xml` (reverted to upstream), `pref_notifications.xml`, `pref_advanced_settings.xml`, `SnoozeActivity.java`, `EditAlertActivity.java`, `Preferences.java`, `Home.java`, `Notifications.java`, `StatusLine.java`
