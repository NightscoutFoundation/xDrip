  ...
  //  private final boolean use_polling = Home.getPreferencesBooleanDefaultFalse(PREF_DEX_COLLECTION_POLLING) && DexCollectionType.hasLibre();
    ...
    public synchronized void setSerialDataToTransmitterRawData(byte[] buffer, int len) {
        ...
        if (XbridgePlus.isXbridgeExtensionPacket(buffer)) {
            ...
            if (((buffer.length > 0) && (buffer[0] == 0x07 || buffer[0] == 0x11 || buffer[0] == 0x15)) || CollectionServiceStarter.isDexBridgeOrWifiandDexBridge()) {
                ...
                //Log.w(TAG, "bufferinBlueReader:" + new String(buffer) );
    ...
    private synchronized void processNewTransmitterData(TransmitterData transmitterData, long timestamp) {
        ...
        if (prefs.getString("btDevice","").equals("blueReader")) {
            ...
            Log.w(TAG,"blueReader-Bat: " + transmitterData.sensor_battery_level + "|" + sensor.latest_battery_level + "|" + prefs.getInt("blueReader_Full_Battery", 3800)); // writing all batinfo to make a curve and Day estimation
        ...
        //Log.w(TAG, "transmitterdata:" +transmitterData.raw_data );
