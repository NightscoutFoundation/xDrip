...
public class SyncingService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (intent != null) {
            ...
            if (ACTION_SYNC.equals(action)) {
                ...
                UserErrorLog.i("CALIBRATION-CHECK-IN: ", "Beginning check in process");
    ...
    private void performCalibrationCheckin(){
        ...
        wl.acquire();
        try {
            UserErrorLog.i("CALIBRATION-CHECK-IN: ", "Wake Lock Acquired");
...
//                ReadData readData = new ReadData(mSerialDevice);
                    ...
                    UserErrorLog.i("CALIBRATION-CHECK-IN: ", "Found " + calRecords.length + " Records!");
                    save_most_recent_cal_record(calRecords);

                } catch (Exception e) {
                    UserErrorLog.wtf("Unhandled exception caught", e);
                } finally {
                    // Close serial
                    try {
                        mSerialDevice.getPorts().get(0).close();
                    } catch (IOException e) {
                        UserErrorLog.e(TAG, "Unable to close", e);
                    }

                }
            } else {
                UserErrorLog.w("CALIBRATION-CHECK-IN: ", "Failed to acquire serial device");
            }
        } finally {
...
//                                 displayTime, array ,batLevel);
                ...
                UserErrorLog.wtf("Unable to read from the dexcom, maybe it will work next time", e);
            } catch (NegativeArraySizeException e) {
                UserErrorLog.wtf("Negative array exception from receiver", e);
            } catch (IndexOutOfBoundsException e) {
                UserErrorLog.wtf("IndexOutOfBounds exception from receiver", e);
    ...
    private boolean acquireSerialDevice() {
        ...
        if (mUsbManager == null) {
            UserErrorLog.w("CALIBRATION-CHECK-IN: ", "USB manager is null");
            return false;
        }

        if (dexcom == null) {
            UserErrorLog.e(TAG, "dex device == null");
            return false;
        }
            ...
            while (foo.hasNext()) {                                                         // let's loop through
                ...
                if (driver != null) {
                    ...
                    if (connection != null) {
                        ...
                        UserErrorLog.i("CALIBRATION-CHECK-IN: ", "CONNECTEDDDD!!");
                        return true;
                    }
                } else {
                    UserErrorLog.w("CALIBRATION-CHECK-IN: ", "Driver was no good");
                }
            }
            UserErrorLog.w("CALIBRATION-CHECK-IN: ", "No usable drivers found");
        } else {
            UserErrorLog.w("CALIBRATION-CHECK-IN: ", "You dont have permissions for that dexcom!!");
        }
        return false;
    }
    ...
    static public boolean isG4Connected(Context c){
        ...
        UserErrorLog.i("USB DEVICES = ", deviceList.toString());
        Iterator<UsbDevice> deviceIterator = deviceList.values().iterator();
        UserErrorLog.i("USB DEVICES = ", String.valueOf(deviceList.size()));
        ...
        while(deviceIterator.hasNext()){
            ...
            if (device.getVendorId() == 8867 && device.getProductId() == 71
                ...
                UserErrorLog.i("CALIBRATION-CHECK-IN: ", "Dexcom Found!");
                return true;
            }
        }
        return false;
    ...
    public UsbDevice findDexcom() {
        UserErrorLog.i("CALIBRATION-CHECK-IN: ", "Searching for dexcom");
        mUsbManager = (UsbManager) getApplicationContext().getSystemService(Context.USB_SERVICE);
        UserErrorLog.i("USB MANAGER = ", mUsbManager.toString());
        ...
        while(deviceIterator.hasNext()){
            ...
            if (device.getVendorId() == 8867 && device.getProductId() == 71
                ...
                UserErrorLog.w("CALIBRATION-CHECK-IN: ", "that was not a dexcom (I dont think)");
            }
        }
        return null;
    }
    ...
    private void broadcastSGVToUI(EGVRecord egvRecord, boolean uploadStatus,
        ...
        UserErrorLog.d(TAG, "Current EGV: " + egvRecord.getBGValue());
