...
public class Sensor extends Model {
  ...
  private volatile static long unitTestStopTime;
    ...
    public synchronized static Sensor create(long starting_at) {
        ...
        return create(starting_at, UUID.randomUUID().toString(), starting_at);
    }

    public synchronized static Sensor create(long starting_at, String uuid) {
        return create(starting_at, uuid, starting_at);
    }

    public synchronized static Sensor create(long starting_at, long started_warmup_at) {
        stopSensor(true); // stop any existing sensor session clamping to the last reading
        return create(starting_at, UUID.randomUUID().toString(), started_warmup_at);
    }

    public synchronized static Sensor create(long starting_at, String uuid, long started_warmup_at) {//KS
        ...
        sensor.started_warmup_at = started_warmup_at;
        if (sensor.started_at > tsl() || sensor.started_warmup_at > tsl()) {
