...
public class NightscoutCallbackTest extends RobolectricTestWithConfig {
    ...
    @Test
    public void onResponse_stillCallsOnSuccess_whenPopulatorThrows() {
        // :: Setup
        AtomicBoolean onSuccessCalled = new AtomicBoolean(false);
        Session session = new Session();
        NightscoutCallback<String> callback = new NightscoutCallback<>(
                "test", session,
                body -> { throw new RuntimeException("populator blew up"); },
                () -> onSuccessCalled.set(true));

        // :: Act
        callback.onResponse(null, Response.success("body"));

        // :: Verify — onSuccess must fire even when populator throws
